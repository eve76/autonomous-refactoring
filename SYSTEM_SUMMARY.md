...
本文件是 `writing/experiment/` 的完整系统总结,目标是:**只读本文件即可从零复现该系统的架构、数学、协议、参数与实验流程**。

> `writing/experiment/` 的 subscription 版本在 2026-08-27 从该实现同步而来。
> 默认 transport 现为 `subscription`:Orchestrator、Analyst、Programmer 的所有
> 模型响应都通过已登录 Claude.ai Pro/Max 的 Claude Code CLI 完成。Orchestrator
> 使用 `--tools ""`、`--max-turns 1` 和 JSON Schema;worker 使用 `--safe-mode`
> 而非会禁用 OAuth/keychain 的 `--bare`。子进程会删除所有 Anthropic API
> key/base-URL 环境变量以防意外切换到 Console 计费。额度耗尽时停止新分派、
> 等待在飞任务结束并以 `subscription_quota_exhausted` 可恢复退出。下文中关于
> 默认 Anthropic SDK Orchestrator、`--bare` 和实际 USD 成本的描述仅适用于原始
> token-save/API transport;subscription 的 USD 数值只是 API 等价估算。
...
agents/
  orchestrator.py              两个同步 LLM 决策点(subscription CLI / 可选 API)
  provider.py                  subscription 认证与隔离 / 可选 API client
...
两者都是 `claude` CLI 子进程。默认 subscription provider 使用 Claude.ai OAuth；可选 DeepSeek provider 下**只有子进程环境**收到 DeepSeek 文档化的变量(见 §11.4)。
...
- **safe mode + 关闭 session 持久化**让每次 dispatch 独立于 CLAUDE.md、hooks、插件、MCP、自定义 agent 和先前会话，同时保留订阅 OAuth。subscription 子进程还会删除 Anthropic API key/token/base URL，避免意外切换到按量 API 计费。
...
|---|---|---|
| orchestrator 模型 | `claude-opus-5` | `deepseek-v4-pro` |
| agent 模型 | `claude-opus-5` | `deepseek-v4-pro[1m]`（包括 Claude Code 内部子代理） |
...
两个数据源:
- `orchestrator_usage.jsonl` — 每次同步 CLI/API 调用一条追加记录(原子 `O_APPEND`),跨 `--resume` 保留;读取时跳过没有终止换行的行。
...
| `orchestrator_usage.jsonl` | 每次 orchestrator CLI/API 调用一条 token 事件 |
...
| 模型 | Anthropic 默认 `claude-opus-5`;DeepSeek 模式用 V4 Pro。二者都与论文的 Opus 4.6 不同,**必须作为实验配置记录** |
