# Dynamic Metrics Refactoring Experiment

A working copy of the multi-agent refactoring system described in the
...
This `dynamic_experiment/` variant adds optional, benchmark-derived dynamic
metrics to the static refactoring penalty. Results are produced only when the
explicit dynamic benchmark mode is selected; no benchmark results are bundled
with this copy.
...
Opus 4.6; this reproduction uses the Claude CLI for the analyst and
programmer subprocesses and the Anthropic-compatible SDK for the
orchestrator. Anthropic is the default provider; DeepSeek is an explicit
alternative using its official compatible API.
...
- **Agent layer** (LLM-based judgment). Three roles:
  - **Orchestrator** — synchronous Anthropic-format SDK calls. Not a
    persistent process; invoked at the thesis's *two* decision points:
    task assignment and stuck-agent evaluation.
...
Synchronous LLM calls (Anthropic-compatible SDK direct):
...
├── DYNAMIC_BENCHMARK_EXTENSION_PLAN.md # Dynamic performance experiment design
├── scripts/
│   └── production_validation.py # Repository-native build/test entrypoint
├── requirements.txt
├── bin/duplo                  # Duplo binary
├── dynamic_metrics/           # Native benchmark collection, parsing, comparison
│
...
│   ├── orchestrator.py        #   sync SDK calls (assign + evaluate_stuck)
...
│   ├── provider.py            #   Anthropic/DeepSeek client + child env
...
| `orchestrator_usage.jsonl` | Append-only token event per orchestrator SDK call; retained across `--resume` |
| `orchestrator_raw_responses.jsonl` | Full raw SDK response and the exact text passed to the Orchestrator parser for every call; contains no API credential |
...
| Item | Status |
|------|--------|
| Agent runtime | Claude CLI + Anthropic-compatible SDK instead of KIRO CLI — the intended deviation. `api_provider` selects Anthropic or DeepSeek without changing the coordination protocol |
...
| Model | Anthropic defaults to `claude-opus-4-7`; optional DeepSeek mode uses `deepseek-v4-pro` for the orchestrator and `deepseek-v4-pro[1m]` for every Claude Code role, including subagents |
...
| `max_run_cost_usd` | 0 | Pinned-price USD dispatch ceiling; 0 disables it and in-flight agents finish |
...
| `api_provider` | `anthropic` | `anthropic` or `deepseek`; both use the Anthropic SDK/wire format |
| `api_base_url` | provider default | Optional Anthropic-compatible endpoint override |
| `api_key_env` | provider default | Environment-variable name containing the key; the secret itself is never stored |
| `orchestrator_model` / `agent_model` | provider-specific | Anthropic: `claude-opus-4-7`; DeepSeek: `deepseek-v4-pro` / `deepseek-v4-pro[1m]` |
...
| `orchestrator_usage_filename` | `orchestrator_usage.jsonl` | Append-only SDK usage events |
...
# 4. Configure one provider (or put the same assignment in .env)
...
Dynamic benchmarks are opt-in: `--dynamic-mode off` preserves the original
static-only gate, `observe` records a comparison without affecting acceptance,
and `enforce` fail-closes when a benchmark is unavailable, non-comparable, or
raises the combined static-plus-dynamic penalty. Before agent dispatch the
integration commit is measured once with seven repetitions. Each gate measures
only its candidate with seven repetitions; after a successful fast-forward,
that candidate measurement atomically becomes the next rolling baseline.
Rejected candidates leave the baseline unchanged. The design is
recorded in [DYNAMIC_BENCHMARK_EXTENSION_PLAN.md](DYNAMIC_BENCHMARK_EXTENSION_PLAN.md).
MongoDB uses Google Benchmark's aggregate-mean `real_time` from the existing
`//src/mongo/db/query:canonical_query_bm` (eight parameterised cases).
FerretDB uses its existing integration `BenchmarkFind` and `BenchmarkInsert`
outputs with Go's `-benchmem`, retaining the seven-repetition arithmetic means
of `ns/op` and `B/op`. FerretDB additionally requires a user-supplied,
dedicated PostgreSQL URL in the configured environment variable; the URL is
redacted from command artifacts.
...
|---|---:|---:|---:|---:|
| Claude Opus 4.7 | 5.00 | 0.50 | 25.00 | 6.25 / 10.00 |
